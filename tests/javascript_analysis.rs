...
fn fixture_path(name: &str) -> String {
    ...
    format!("{manifest_dir}/tests/fixtures/javascript/{name}")
}

#[test]
fn simple_function_metrics() {
    let report = analyze_file(fixture_path("simple_function.js")).unwrap();
...
fn nested_control_flow_metrics() {
    let report = analyze_file(fixture_path("nested_control_flow.js")).unwrap();
    assert_eq!(report.functions.len(), 1);
...
fn boolean_operators_metrics() {
    let report = analyze_file(fixture_path("boolean_operators.js")).unwrap();
...
fn else_if_chain_metrics() {
    let report = analyze_file(fixture_path("else_if_chain.js")).unwrap();
    assert_eq!(report.functions.len(), 1);
...
#[test]
fn arrow_functions_metrics() {
    let report = analyze_file(fixture_path("arrow_functions.js")).unwrap();
    assert_eq!(report.functions.len(), 1);
...
fn recursion_metrics() {
    let report = analyze_file(fixture_path("recursion.js")).unwrap();
    assert_eq!(report.functions.len(), 1);
...
#[test]
fn language_detected_as_javascript() {
    let report = analyze_file(fixture_path("simple_function.js")).unwrap();
    assert_eq!(report.language, arborist::Language::JavaScript);
}

#[test]
fn path_populated_in_report() {
    let path = fixture_path("simple_function.js");
    let report = analyze_file(&path).unwrap();
    assert!(
        report.path.contains("simple_function.js"),
...
fn match_switch_metrics() {
    let report = analyze_file(fixture_path("match_switch.js")).unwrap();
    assert_eq!(report.functions.len(), 1);
