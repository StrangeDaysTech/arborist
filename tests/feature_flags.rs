...
fn enabled_rust_works() {
    ...
    assert!(
        result.is_ok(),
        "Rust analysis should work when feature is enabled"
    );
}
...
fn enabled_python_works() {
    ...
    assert!(
        result.is_ok(),
        "Python analysis should work when feature is enabled"
    );
}
...
fn disabled_csharp_returns_error() {
    ...
    assert!(matches!(
        result,
        Err(arborist::ArboristError::LanguageNotEnabled { .. })
    ));
}
...
fn disabled_c_returns_error() {
    let result = arborist::analyze_source(
...
fn enabled_kotlin_works() {
    ...
    assert!(
        result.is_ok(),
        "Kotlin analysis should work when feature is enabled"
    );
}
...
fn enabled_swift_works() {
    ...
    assert!(
        result.is_ok(),
        "Swift analysis should work when feature is enabled"
    );
}
