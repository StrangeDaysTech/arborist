...
fn fixture_path(name: &str) -> String {
    ...
    format!("{manifest_dir}/tests/fixtures/kotlin/{name}")
}

#[test]
fn simple_function_metrics() {
    let report = analyze_file(fixture_path("simple_function.kt")).unwrap();
...
fn nested_control_flow_metrics() {
    let report = analyze_file(fixture_path("nested_control_flow.kt")).unwrap();
    assert_eq!(report.functions.len(), 1);
...
fn boolean_operators_metrics() {
    let report = analyze_file(fixture_path("boolean_operators.kt")).unwrap();
...
fn else_if_chain_metrics() {
    let report = analyze_file(fixture_path("else_if_chain.kt")).unwrap();
    assert_eq!(report.functions.len(), 1);
    ...
    // base(1) + 3 if_expressions + 0 (else not counted) = 4
...
fn lambda_nested_metrics() {
    let report = analyze_file(fixture_path("lambda_nested.kt")).unwrap();
    assert_eq!(report.functions.len(), 1);
...
fn match_switch_metrics() {
    let report = analyze_file(fixture_path("match_switch.kt")).unwrap();
    assert_eq!(report.functions.len(), 1);

    let f = &report.functions[0];
    assert_eq!(f.name, "classify");
    // when(+1, nesting=0) + else_entry(+1) = 2
    assert_eq!(f.cognitive, 2, "cognitive complexity for classify");
    // base(1) + 4 when_entry arms = 5
    assert_eq!(f.cyclomatic, 5, "cyclomatic complexity for classify");
    assert_eq!(f.sloc, 8, "sloc for classify");
}

#[test]
fn recursion_metrics() {
    let report = analyze_file(fixture_path("recursion.kt")).unwrap();
    assert_eq!(report.functions.len(), 1);

    let f = &report.functions[0];
    assert_eq!(f.name, "fibonacci");
    // if(+1) — recursion not detected (Kotlin grammar has no "function" field on call_expression)
...
#[test]
fn language_detected_as_kotlin() {
    let report = analyze_file(fixture_path("simple_function.kt")).unwrap();
    assert_eq!(report.language, arborist::Language::Kotlin);
}

#[test]
fn path_populated_in_report() {
    let path = fixture_path("simple_function.kt");
    let report = analyze_file(&path).unwrap();
    assert!(
        report.path.contains("simple_function.kt"),
