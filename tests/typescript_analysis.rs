...
fn fixture_path(name: &str) -> String {
    ...
    format!("{manifest_dir}/tests/fixtures/typescript/{name}")
}

#[test]
fn simple_function_metrics() {
    let report = analyze_file(fixture_path("simple_function.ts")).unwrap();
...
fn nested_control_flow_metrics() {
    let report = analyze_file(fixture_path("nested_control_flow.ts")).unwrap();
    assert_eq!(report.functions.len(), 1);
...
fn boolean_operators_metrics() {
    let report = analyze_file(fixture_path("boolean_operators.ts")).unwrap();
...
fn else_if_chain_metrics() {
    let report = analyze_file(fixture_path("else_if_chain.ts")).unwrap();
    assert_eq!(report.functions.len(), 1);
...
#[test]
fn arrow_closures_metrics() {
    let report = analyze_file(fixture_path("arrow_closures.tsx")).unwrap();
    assert_eq!(report.functions.len(), 1);
...
fn recursion_metrics() {
    let report = analyze_file(fixture_path("recursion.ts")).unwrap();
    assert_eq!(report.functions.len(), 1);
...
#[test]
fn language_detected_as_typescript() {
    let report = analyze_file(fixture_path("simple_function.ts")).unwrap();
    assert_eq!(report.language, arborist::Language::TypeScript);
}

#[test]
fn path_populated_in_report() {
    let path = fixture_path("simple_function.ts");
    let report = analyze_file(&path).unwrap();
    assert!(
        report.path.contains("simple_function.ts"),
...
fn match_switch_metrics() {
    let report = analyze_file(fixture_path("match_switch.ts")).unwrap();
    assert_eq!(report.functions.len(), 1);
